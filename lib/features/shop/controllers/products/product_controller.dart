...
class ProductController extends GetxController {
  ...
  String getProductPrice(ProductModel products) {
    ...
    if (products.productType == ProductType.single.toString()) {
      return (products.salePrices > 0 ? products.salePrices : products.price)
          .toString();
    } else {
      // Calculate the smallest and largest prices among variable
      for (var variable in products.productVariations!) {
        double priceToConsider = variable.salePrice > 0.0
            ? variable.salePrice
            : variable.price;
  ...
  String? calculaSaletePercentage(double originalPrice, double? salePrice) {
